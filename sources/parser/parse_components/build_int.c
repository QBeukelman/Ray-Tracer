...
/*   Updated: 2025/05/05 21:18:28 by quentinbeuk   ########   odam.nl         */
/*                                                                            */
/* ************************************************************************** */
 ...
 * The parsed value is validated to ensure it lies within the inclusive range
 * 	[0, VAL_LIMIT].
 * 
 * If both the format and the range are valid, the result is stored in `*val`
 * 	and the function returns true. If the format is invalid, it calls
 * 	`show_error` with `E_INVALID_C`. If the value is out of range, it calls
 * 	`show_error` with `E_OUT_OF_RANGE`.
 *
 * @param val Pointer to an integer where the parsed value will be stored if
 * 	valid.
